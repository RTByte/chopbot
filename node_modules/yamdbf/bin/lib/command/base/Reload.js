...
class Reload extends _Command2.default {
	constructor(bot) {
		super(bot, {
			name: 'reload',
	...
	action(message, args, mentions) {
		return new Promise(function ($return, $error) {
			const start = (0, _performanceNow2.default)();
			const command = this.bot.commands.findByNameOrAlias(args[0]);

			if (args[0] && !command) return $return(this._respond(message, `Command "${ args[0] }" could not be found.`).then(response => response.delete(5 * 1000)));

			if (command) this.bot.loadCommand(command.name);else this.bot.loadCommand('all');

			const end = (0, _performanceNow2.default)();
			const name = command ? command.name : null;
			const text = name ? ` "${ name }"` : 's';
			return $return(this._respond(message, `Command${ text } reloaded. (${ (end - start).toFixed(3) } ms)`).then(response => response.delete(5 * 1000)));
		}.$asyncbind(this));
	}
}
