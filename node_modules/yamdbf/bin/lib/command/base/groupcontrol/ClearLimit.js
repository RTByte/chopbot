...
class ClearLimit extends _Command2.default {
	constructor(bot) {
		super(bot, {
			name: 'clearlimit',
	...
	action(message, args, mentions) {
		return new Promise(function ($return, $error) {
			...
			if (!commandName) return $return(this._respond(message, `You must provide a command to clear limits for.`));
			let command = this.bot.commands.find(c => (0, _Util.normalize)(c.name) === (0, _Util.normalize)(commandName));
			...
			return $return(this._respond(message, `Successfully cleared role limits for command: \`${ command.name }\``));
		}.$asyncbind(this));
	}
}
exports.default = ClearLimit;