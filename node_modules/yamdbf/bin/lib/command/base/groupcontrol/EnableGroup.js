...
class EnableGroup extends _Command2.default {
	constructor(bot) {
		super(bot, {
			name: 'enablegroup',
	...
	action(message, args, mentions) {
		return new Promise(function ($return, $error) {
			...
			if (!args[0]) sendError(err.NO_GROUP);else if (!this.bot.commands.groups.includes(args[0])) sendError(err.NO_EXIST);else if (args[0] === 'base' || !this.bot.guildStorages.get(message.guild).getSetting('disabledGroups').includes(args[0])) sendError(err.ENABLED);
			...
			this._respond(message, `**Enabled command group "${ args[0] }"**`).then(response => response.delete(5 * 1000));
			return $return();
		}.$asyncbind(this));
	}
}
exports.default = EnableGroup;